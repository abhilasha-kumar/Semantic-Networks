...
```{r}
item_mean = group_by(sem, pathlength, ItemNo) %>%
  summarise_at(vars(zRT_trim), mean)
...
```{r}
bigram = read.csv("w2_.txt", header = TRUE, sep = "\t", 
                  stringsAsFactors = FALSE)
colnames(bigram) = c("f", "prime_word", "target_word")

bigram$prime_word = paste(bigram$prime_word, "")
bigram$target_word = paste(bigram$target_word, "")


sem$prime_word = tolower(sem$prime_word)
sem$target_word = tolower(sem$target_word)

new_sem = dplyr::inner_join(sem, bigram, by = c("prime_word", 
                                          "target_word"))

newsem = newsem[order(newsem$ItemNo),]
...
item_agg %>% 
  ggplot(aes(x = pathlengthfac, y = Trials, group = Type, fill = Type))+
#  geom_smooth(method = "loess")+
 geom_bar(stat = "identity", position = "dodge", width = 0.7, color= "black")+
  geom_errorbar(aes(ymin=Trials - ci, ymax=Trials + ci), 
             width=.05, color = "gray30", 
...
options(contrasts = c('contr.sum', 'contr.poly'))
 ...
 prime_effect = cld(sem_lsm, alpha = 0.05, 
                 adjust = "tukey", details = TRUE)
 library(knitr)
kable(subset(prime_effect$comparisons,prime_effect$comparisons$p.value <.5))
...
``` {r  }
## aggregate per subject all IVs and DVs
meanRT = group_by(sem_firsttrim, ItemNo) %>%
  summarise_at(vars(rt), mean)
colnames(meanRT) = c("ItemNo", "MeanRT")

sdRT = group_by(sem_firsttrim, ItemNo) %>%
  summarise_at(vars(rt), sd)
colnames(sdRT) = c("ItemNo", "sdRT")

RT_agg = merge(meanRT, sdRT, by = "ItemNo")

## merge aggregate info with long data
itemsem_z = merge(sem_firsttrim, RT_agg, by = "ItemNo", all.x = T)

## person and grand-mean centered scores using original and aggregate
library(dplyr)
itemsem_z = itemsem_z %>% mutate(zRT = (rt - MeanRT)/sdRT)
                 
## checking: subject level means should be zero

item_pic = group_by(itemsem_z, ItemNo) %>%
  summarise_at(vars(zRT), mean)

```
## z-RT Distribution
``` {r  }
ggplot(itemsem_z, aes(x = zRT))+
geom_histogram(binwidth = 0.5, color = "gray4", fill = "goldenrod1")+
    theme_few()+
  xlab("z RT") + ylab("Count") + 
  ggtitle("z-RT Histogram for above 250 ms & <2s Trials")
...
``` {r  }
itemsem_z_trimmed = subset(itemsem_z, itemsem_z$zRT < 3 & itemsem_z$zRT > -3)

ggplot(itemsem_z_trimmed, aes(x = zRT))+
geom_histogram(binwidth = 0.25, color = "gray4", fill = "goldenrod1")+
    theme_few()+
  xlab("Raw RT") + ylab("Count") + 
  ggtitle("Trimmed (3 SD) z-RT Histogram for above 250 ms & <2s Trials")
...
## aggregate per subject all IVs and DVs
meanRT_trim = group_by(itemsem_z_trimmed, ItemNo) %>%
  summarise_at(vars(rt), mean)
colnames(meanRT_trim) = c("ItemNo", "MeanRT_trim")

sdRT_trim = group_by(itemsem_z_trimmed, ItemNo) %>%
  summarise_at(vars(rt), sd)
colnames(sdRT_trim) = c("ItemNo", "sdRT_trim")

RT_agg_trim = merge(meanRT_trim, sdRT_trim, by = "ItemNo")

## merge aggregate info with long data
itemnew_sem_z = merge(itemsem_z_trimmed, RT_agg_trim, by = "ItemNo", all.x = T)

## person and grand-mean centered scores using original and aggregate
library(dplyr)
itemnew_sem_z = itemnew_sem_z %>% 
  mutate(zRT_trim = (rt - MeanRT_trim)/sdRT_trim)
                 
## checking: item level means should be zero

item_pic = group_by(itemnew_sem_z, ItemNo) %>%
  summarise_at(vars(zRT_trim), mean)
```

## Aggregating zRT
``` {r}
itemz_sem_rt = group_by(itemnew_sem_z, ItemNo ) %>%
  summarise_at(vars(zRT_trim), mean)

itemz_sem_rt_rel = group_by(itemnew_sem_z, ItemNo, pathlength, Type ) %>%
  summarise_at(vars(zRT_trim), mean)

itemz_sem_rt_agg = group_by(itemnew_sem_z, pathlength ) %>%
  summarise_at(vars(zRT_trim), mean)

itemz_rmisc = Rmisc::summarySE(itemnew_sem_z, 
                        measurevar = "zRT_trim",
                        groupvars = c("pathlength"))

itemz_sem_rt_rel_agg = group_by(itemnew_sem_z, pathlength, Type ) %>%
  summarise_at(vars(zRT_trim), mean)

itemz_sem_rt_rel_agg_rmisc = Rmisc::summarySE(itemnew_sem_z, 
                        measurevar = "zRT_trim",
                        groupvars = c("pathlength", "Type"))
```
...
## ANOVA
```{r}
itemz_sem_rt$pathlengthfac = ordered(as.factor(as.character(itemz_sem_rt$pathlength)), 
                            levels = c("1", "2", "3", "4", "6", "15"))
itemz_sem_rt$ItemNo = as.factor(itemz_sem_rt$ItemNo)
...
itemz_sem_rt_rel_agg_rmisc$pathlengthfac = ordered(as.factor(as.character(itemz_sem_rt_rel_agg_rmisc$pathlength)),                             levels = c("1", "2", "3", "4", "6", "15"))

itemz_sem_rt_rel_agg_rmisc$zRT_trim = as.numeric(itemz_sem_rt_rel_agg_rmisc$zRT_trim)

library(ggplot2)
library(ggthemes)

itemz_sem_rt_rel_agg_rmisc %>% 
  ggplot(aes(x = pathlengthfac, y = zRT_trim, group = Type, color = Type))+
  geom_point()+
#  geom_smooth(method = "loess")+
geom_line()+
