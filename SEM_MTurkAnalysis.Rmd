...
```{r}
relunrel_aov = aov(data = sem_decision, proportion ~ pathlengthfac*actual + 
                     Error(subject/(pathlengthfac*actual)))
...
m0 = lmer(data = sem, rt ~ 1 + (1|subject) + (1|trial_index))
summary(m0)
reghelper::ICC(m0)

contrasts(sem$pathlengthfac) = contr.treatment(6, base = 1)

m1 = lmer(data = sem, rt ~ pathlengthfac + (1|subject) + (1|trial_index))
summary(m1)

m2 = lmer(data = sem, rt ~ pathlengthfac + (pathlengthfac|subject))
summary(m2)
```
